...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('G1', 'Nama item')
			->setCellValue('H1', 'Jumlah')
			->setCellValue('I1', 'Harga Pembelian');
...
while ($row = mysql_fetch_array($query)) {
            ...
            ->setCellValue('D'.$i, 'Rp. '.number_format($row['purchase_total']))
            ->setCellValue('E'.$i, $row['supplier_name'])
			->setCellValue('F'.$i, $row['branch_name'])
			// ->setCellValue('H'.$i, 'Rp. '.number_format($svc))
			->setCellValue('G'.$i, $row['item_name'])
			->setCellValue('H'.$i, $row['purchase_qty'])
			->setCellValue('I'.$i, 'Rp. '.number_format($row['purchase_price']));
			$i++;
			$j++;
}
